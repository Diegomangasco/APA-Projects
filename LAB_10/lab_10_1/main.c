...
int main() {
    ...
    printf("\nCollana massima: %d",max);
    return 0;
}
void alloc(stone *vett, int *****Z) //allocazione delle matrici
{
    int i,j,k;
...
void inizializza(stone *vett, int ****Z)    //inizializzo tutto a -1
{
    int i,j,k,t;
...
{
    int one,two;
    if (z<=0)   //finito gli zaffiri
        return 0;
    if(Z[z][t][s][r]!=-1) return Z[z][t][s][r]; //se ho già calcolato il costo di una collana iniziante con z zaffiri, t topazi, s smeraldi e r rubini, ritotno subito il risultato
    one=fz(z-1,t,s,r,R,S,T,Z);  //ricorro aggiungendo uno zaffiro
    two=fr(z-1,t,s,r,R,S,T,Z);  //ricorro aggiungendo un rubino
    if(one>two) Z[z][t][s][r]=one+1;    //memorizzo la soluzione migliore
    else Z[z][t][s][r]=two+1;
    return Z[z][t][s][r];   //ritorno la soluzione migliore
}
int fr(int z, int t, int s, int r, int **** R, int ****S, int ****T, int ****Z)
{
    int one,two;
